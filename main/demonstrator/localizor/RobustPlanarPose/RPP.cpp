...
bool Rpp(const Mat &model_3D, const Mat &iprts, Mat &Rlu, Mat &tlu, int &it1,
		double &obj_err1, double &img_err1) {
	...
	for (unsigned int i = 0; i < sol.size(); i++) {
		ObjPose(model_3D, iprts, sol[i].R, Rlu, sol[i].t, it1, obj_err1,
				img_err1);
...
void ObjPose(const Mat _P, Mat Qp, Mat initR, Mat &R, Mat &t, int &it,
		double &obj_err, double &img_err) {
	...
	for (int i = 0; i < n; i++) {
		...
		double xx = (Qproj.at<double>(0, 0) / Qproj.at<double>(2, 0))
				- Qp.at<double>(0, 0);
		double yy = (Qproj.at<double>(1, 0) / Qproj.at<double>(2, 0))
				- Qp.at<double>(1, 0);
...
void AbsKernel(Mat P, Mat Q, const vector<Mat> &F, const Mat &G, Mat &R, Mat &t,
		Mat &Qout, double &err2) {
	int n = P.cols;
...
Quaternion Quaternion_byAngleAndVector(double q_angle, const Vec3d &q_vector) {
	...
	Quaternion Q_ = Quaternion_byVectorAndScalar(rotation_axis,
			cos(q_angle * 0.5));
...
bool Get2ndPose_Exact(const Mat &v, const Mat &P, const Mat &R, const Mat &t,
		vector<Solution> &ret) {
...
Mat Xform(const Mat &P, const Mat &R, const Mat &t) {
	...
	for (int i = 0; i < P.cols; i++) {
		...
		ret.at<double>(0, i) = R.at<double>(0, 0) * x + R.at<double>(0, 1) * y
				+ R.at<double>(0, 2) * z + t.at<double>(0, 0);
		ret.at<double>(1, i) = R.at<double>(1, 0) * x + R.at<double>(1, 1) * y
				+ R.at<double>(1, 2) * z + t.at<double>(1, 0);
		ret.at<double>(2, i) = R.at<double>(2, 0) * x + R.at<double>(2, 1) * y
				+ R.at<double>(2, 2) * z + t.at<double>(2, 0);
	}
...
bool GetRfor2ndPose_V_Exact(const Mat &v, const Mat &P, const Mat &R,
...
void GetRotationY_wrtT(const Mat &v, const Mat &p, const Mat &t, const Mat &Rz,
	...
	for (int i = 0; i < v.cols; i++) {
		...
		opt_t.at<double>(0, 0) += (((v11 - 1) * r2 + v12 * r5 + v13 * r8) * py
				+ (-(v11 - 1) * r1 - v12 * r4 - v13 * r7) * px
				+ (-(v11 - 1) * r3 - v12 * r6 - v13 * r9) * pz);
		opt_t.at<double>(0, 1) += ((2 * (v11 - 1) * r1 + 2 * v12 * r4
				+ 2 * v13 * r7) * pz
				+ (-2 * (v11 - 1) * r3 - 2 * v12 * r6 - 2 * v13 * r9) * px);
		opt_t.at<double>(0, 2) += ((v11 - 1) * r1 + v12 * r4 + v13 * r7) * px
				+ ((v11 - 1) * r3 + v12 * r6 + v13 * r9) * pz
				+ ((v11 - 1) * r2 + v12 * r5 + v13 * r8) * py;

		opt_t.at<double>(1, 0) += ((v21 * r2 + (v22 - 1) * r5 + v23 * r8) * py
				+ (-v21 * r1 - (v22 - 1) * r4 - v23 * r7) * px
				+ (-v21 * r3 - (v22 - 1) * r6 - v23 * r9) * pz);
		opt_t.at<double>(1, 1) += ((2 * v21 * r1 + 2 * (v22 - 1) * r4
				+ 2 * v23 * r7) * pz
				+ (-2 * v21 * r3 - 2 * (v22 - 1) * r6 - 2 * v23 * r9) * px);
		opt_t.at<double>(1, 2) += (v21 * r1 + (v22 - 1) * r4 + v23 * r7) * px
				+ (v21 * r3 + (v22 - 1) * r6 + v23 * r9) * pz
				+ (v21 * r2 + (v22 - 1) * r5 + v23 * r8) * py;

		opt_t.at<double>(2, 0) += ((v31 * r2 + v32 * r5 + (v33 - 1) * r8) * py
				+ (-v31 * r1 - v32 * r4 - (v33 - 1) * r7) * px
				+ (-v31 * r3 - v32 * r6 - (v33 - 1) * r9) * pz);
		opt_t.at<double>(2, 1) += ((2 * v31 * r1 + 2 * v32 * r4
				+ 2 * (v33 - 1) * r7) * pz
				+ (-2 * v31 * r3 - 2 * v32 * r6 - 2 * (v33 - 1) * r9) * px);
		opt_t.at<double>(2, 2) += (v31 * r1 + v32 * r4 + (v33 - 1) * r7) * px
				+ (v31 * r3 + v32 * r6 + (v33 - 1) * r9) * pz
...
//    double z[10];
	...
	for (unsigned int i = 0; i < at.size(); i++) {
		...
		double tMaxMin = (4 * a4 * at[i] * at[i] * at[i]
				+ 3 * a3 * at[i] * at[i] + 2 * a2 * at[i] + a1);
